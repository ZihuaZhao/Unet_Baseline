...
class BCELoss_RaterWise(nn.Module):
    def __init__(self, **kwargs):
        super(BCELoss_RaterWise, self).__init__()
        self.bce_loss = nn.BCEWithLogitsLoss()

    def forward(self, output, target):
        losses = []
        for i in range(len(output)):
            losses.append(self.bce_loss(output[i], target[i]).reshape(1,))
