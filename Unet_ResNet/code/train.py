...
parser.add_argument('--train_dataroot', default='/remote-home/zhaozh/dataset/DiscRegion', type=str, help='path to train data')
parser.add_argument('--test_dataroot', default='/remote-home/zhaozh/dataset/DiscRegion', type=str, help='path to test or val data')

parser.add_argument('--outfile_root', default='/remote-home/zhaozh/Unet_ResNet/output_fusion_05', type=str)
parser.add_argument('--val_root', default='/val', type=str, help='directory to save run log')
parser.add_argument('--log_root', default='/log', type=str, help='directory to save run log')
parser.add_argument('--snapshot_root', default='/snapshot', type=str, help='path to checkpoint or snapshot')
parser.add_argument('--output_root', default='/results', type=str, help='path to saliency map')
...
mkdir_if_missing(args.outfile_root)

snapshot_root = args.outfile_root + args.snapshot_root
output_root = args.outfile_root + args.output_root
log_root = args.outfile_root + args.log_root
val_root = args.outfile_root + args.val_root

log_name = os.path.join(log_root, 'log_%s-%s.txt' % (args.phase,time.strftime("%Y-%m-%d-%H-%M-%S")))
...
"""""""""""~~~ dataset loader ~~~"""""""""
mkdir_if_missing(snapshot_root)
mkdir_if_missing(output_root)
mkdir_if_missing(log_root)
mkdir_if_missing(val_root)
...
if args.param is True:      # load pretrain or checkpoint
    ckpt_path1 = os.path.join(snapshot_root, 'snapshot_iter_'+ str(parameters['snap_num'])+'.pth')
    ckpt_path2 = os.path.join(snapshot_root, 'six_snapshot_iter_' + str(parameters['snap_num']) + '.pth')
    ckpt_path3 = os.path.join(snapshot_root, 'att_snapshot_iter_' + str(parameters['snap_num']) + '.pth')
...
training = Trainer(
        ...
        outpath=snapshot_root,
        sshow=parameters['sshow'],
        step_size=config.STEP_SIZE,
        gamma=config.GAMMA,
        log_name=log_name,
        val_out_path=[val_root, args.test_dataroot],
    )
